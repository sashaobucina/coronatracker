...
export default function Home(props) {
  ...
  const { main, dispatch } = useContext(AppContext);
  const { countries, data, tabIndex } = main;
  const { fetched, match, topContributors, validCountries } = props;
  ...
  function clearState() {
    dispatch({
      type: "update-main",
      payload: { countries: [], data: [], tabIndex: 0 },
    });
  }
  ...
  function showFrontPage() {
    return data.length === 0 ? (
      ...
      <GraphBundle country={countries[tabIndex]} data={data[tabIndex]} />
    );
  }
      ...
      <Grid container direction="row" alignItems="center">
        ...
        <Grid item xs={10} sm={10} md={10} lg={10}>
          <Typography
            ...
            style={{ marginTop: 40 }}
          >
            Tracking COVID-19 movements and trends - search <b>"Global"</b> to
            get world view
          </Typography>
        </Grid>
        <Grid item xs={1} sm={1} md={1} lg={1} />
